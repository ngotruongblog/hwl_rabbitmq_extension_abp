...
namespace Hwl.RabbitMQ
{
    ...
    [ExposeServices(typeof(IRabbitMqMessageConsumer), typeof(RabbitMqMessageConsumerExtension))]
    public class RabbitMqMessageConsumerExtension : IRabbitMqMessageConsumer, ITransientDependency, IDisposable, IBonusInfo
    {
        public ILogger<RabbitMqMessageConsumerExtension> Logger { get; set; }
        ...
        public RabbitMqMessageConsumerExtension(
            ...
            IOptions<HandleMessageOptions> handleMessageOptions)
        {
            ...
            Logger = NullLogger<RabbitMqMessageConsumerExtension>.Instance;
        ...
        protected virtual async Task TryCreateChannelAsync()
        {
            ...
            try
            {
                ...
                if (IsExistHandler(HandleMessageOptions.Handlers))
                {
        ...
        protected virtual bool IsExistHandler(ITypeList<IHandleMessage> handlers)
        {
            var flag = false;
            foreach (var handler in handlers)
            {
                ...
                foreach (var @interface in interfaces)
                {
                    ...
                    {
                        var typeMsg = genericArgs[0];
                        if(typeMsg.FullName == this.FullName)
                        {
                            flag = true;
                        }
                    }
        ...
        public void SetTypeMessage(Type messageType)
        {
